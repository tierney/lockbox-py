...
lbm.define('lbm.ui', function() {
   ...
   */
  var MenuItem = lbm.ui.define('div');

  /**
   * Creates a new menu separator element.
   * @return {lbm.ui.MenuItem}
   */
  MenuItem.createSeparator = function() {
    var el = lbm.doc.createElement('hr');
  ...
  MenuItem.prototype = {
    ...
    set command(command) {
      ...
      if (typeof command == 'string' && command[0] == '#') {
        ...
        lbm.ui.decorate(command, Command);
      }
    ...
    handleMouseUp_: function(e) {
      if (!this.disabled && !this.isSeparator()) {
        ...
        if (lbm.dispatchSimpleEvent(this, 'activate', true, true)) {
   ...
   * @type {boolean}
   */
  lbm.defineProperty(MenuItem, 'disabled', lbm.PropertyKind.BOOL_ATTR);

  /**
   * Whether the menu item is hidden or not.
   * @type {boolean}
   */
  lbm.defineProperty(MenuItem, 'hidden', lbm.PropertyKind.BOOL_ATTR);

  /**
   * Whether the menu item is selected or not.
   * @type {boolean}
   */
  lbm.defineProperty(MenuItem, 'selected', lbm.PropertyKind.BOOL_ATTR);

  /**
   * Whether the menu item is checked or not.
   * @type {boolean}
   */
  lbm.defineProperty(MenuItem, 'checked', lbm.PropertyKind.BOOL_ATTR);
