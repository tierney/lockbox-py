...
SQL_SCHEMA = os.path.join(os.path.dirname(__file__), 'SDB.sql')
  ...
  def unshare_file(self, server_path, email):
    ...
    with conn:
      conn.execute('''DELETE FROM file_permission WHERE
                ...
                file_id = (SELECT id FROM file_journal WHERE server_path = ?)''',
                   (email, server_path))
      conn.commit()
    conn.close()

def test_reset():
  s = SQLiteHelper(os.path.expanduser('~/.lockbox'), reset=True)
...
def test_insert_user_loc_key_insert_only():
  s = SQLiteHelper(os.path.expanduser('~/.lockbox'), reset=True)
  s.insert_user_loc_key('tierney@cs.nyu.edu','Macbook Pro',
                        'somethingthatissupposedtorepresentapublickey')
