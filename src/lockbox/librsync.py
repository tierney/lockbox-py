...
class LikeFile:
  ...
  def read(self, length = -1):
    ...
    NB: Can only call read once were opened LikeFile (i.e., no seek(0) method.)
    """
    if length == -1:
      while not self.eof:
...
if __name__ == '__main__':
  ...
  with tempfile.NamedTemporaryFile(delete=False) as v1_fh:
    v1_fh.write('hello world')
    v1_name = v1_fh.name

  with tempfile.NamedTemporaryFile(delete=False) as v2_fh:
    v2_fh.write('hello new world and some more!')
    v2_name = v2_fh.name

  print v1_name
  print v2_name

  with open(v1_name) as v1:
    sigfile = SigFile(v1)
    sig = binascii.b2a_base64(sigfile.read())

  from simpledb import _sha1_of_string
  print 'Upload these to the cloud : filepathhash(%s) PGP(filename): PGP(%s) ' \
      'SHA1(PGP(file)): %s.' % (_sha1_of_string(v1_name), v1_name, 'TODO')

  with open(v2_name) as v2:
    sigfile_v2 = SigFile(v2)
    sig_v2 = binascii.b2a_base64(sigfile_v2.read())

  with open(v2_name) as v2:
    deltafile = DeltaFile(binascii.a2b_base64(sig), v2)
    delta = binascii.b2a_base64(deltafile.read())
  ...
  print 'Upload these to S3: SHA1(PGP(delta(file))): (%s) pgp_delta (%s)' % \
    ...
    #   # Test
    #   print 'base: (%s)' % basefh.read()
    #   basefh.seek(0)

    #   # Initial data.
    #   tmpfh.write("hello world")
    #   tmpfh.seek(0)
    #   print tmpfh.read()
    #   tmpfh.seek(0)
    ...
    #   sg = SigGenerator()
    #   tmpfh.seek(0)
    #   sg.update(tmpfh.read())
    #   bin_sig = sg.getsig()
    #   print '(%s)' % binascii.b2a_base64(bin_sig)
    #   print '(%s)' % bin_sig

    #   # Append some more data.
    #   tmpfh.read()
    #   tmpfh.write(' and some more!')
    #   tmpfh.seek(0)
    #   print 'Appended data file: (%s).' % tmpfh.read()
    #   tmpfh.seek(0)

    #   # Try to calculate the DeltaFile.
    #   print 'Delta file.'
    #   tmpfh.seek(0)
    #   delta = DeltaFile(bin_sig, tmpfh)
    #   # bin_delta = delta.read()
    #   # print '(%s)' % binascii.b2a_base64(bin_delta)
    #   # delta.seek(0)

    #   # Try to patch the file.
    #   base_file = file(basefh.name)
    #   patch = PatchedFile(base_file, delta)
    #   # print dir(patch)
    #   print 'patch: (%s)' % patch.read()

      # while True:
      #   buf = patch.read(blocksize)
      #   if not buf:
      #     break
      #   print buf
    ...
    #     self.setdata()
      ...
      # tmpfh.seek(0)
      # sg.update(tmpfh.read())
      # print '(%s)' % binascii.b2a_base64(sg.getsig())
